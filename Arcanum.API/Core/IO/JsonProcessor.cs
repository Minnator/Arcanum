...
namespace Arcanum.API.Core.IO; // Exposing this via API
...
public interface IJsonProcessor : IService
{
    ...
    /// </summary>
    Task<string> SerializeAsync<T>(T value, JsonSerializationRules? rules = null, CancellationToken cancellationToken = default);

    /// <summary>
    /// Asynchronously deserializes the JSON string to the specified .NET type using custom rules.
    /// </summary>
    Task<T?> DeserializeAsync<T>(string json, JsonDeserializationRules? rules = null, CancellationToken cancellationToken = default);

    /// <summary>
    /// Asynchronously deserializes JSON from a stream to the specified .NET type using custom rules.
    /// </summary>
    Task<T?> DeserializeAsync<T>(Stream utf8JsonStream, JsonDeserializationRules? rules = null, CancellationToken cancellationToken = default);

    /// <summary>
    /// Asynchronously serializes the specified object to a UTF-8 JSON stream using custom rules.
    /// </summary>
    Task SerializeAsync<T>(Stream utf8JsonStream, T value, JsonSerializationRules? rules = null, CancellationToken cancellationToken = default);
}
